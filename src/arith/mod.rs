    ...
    where T: HasAfEnum
{
    ...
    fn not(self) -> Self::Output {
        let mut temp: i64 = 0;
        unsafe {
            let err_val = af_not(&mut temp as MutAfArray,
                                 self.get() as AfArray);
...
unary_func!("Truncate the values in an Array", trunc, af_trunc, AbsOutType);
unary_func!("Computes the sign of input Array values", sign, af_sign, AbsOutType);
...
unary_func!("Compute e raised to the power of value -1", expm1, af_expm1, AbsOutType);
unary_func!("Compute error function value", erf, af_erf, AbsOutType);
unary_func!("Compute the complementary error function value", erfc, af_erfc, AbsOutType);

unary_func!("Compute logarithm base 10", log10, af_log10, AbsOutType);
unary_func!("Compute the logarithm of input Array + 1", log1p, af_log1p, AbsOutType);
...
unary_func!("Compute the logarithm of absolute values of gamma function",
            lgamma, af_lgamma, AbsOutType);
...
unary_func!("Compute e raised to the power of value", exp, af_exp, UnaryOutType);
...
unary_func!("Extract real values from a complex Array", real, af_real, AbsOutType);
unary_func!("Extract imaginary values from a complex Array", imag, af_imag, AbsOutType);
unary_func!("Create a complex Array from real Array", cplx, af_cplx, ComplexOutType);
unary_func!("Compute the complex conjugate", conjg, af_conjg, ComplexOutType);
unary_func!("Compute two raised to the power of value", pow2, af_pow2, UnaryOutType);
unary_func!("Compute the factorial", factorial, af_factorial, UnaryOutType);
...
macro_rules! binary_func {
    ($doc_str: expr, $fn_name: ident, $ffi_fn: ident) => (
        ...
        #[allow(unused_mut)]
        pub fn $fn_name<A, B>(lhs: &Array<A>,
                              rhs: &Array<B>,
                              batch: bool) -> Array< A::Output >
...
pub trait Convertable {
    ...
    fn convert(&self) -> Array< Self::OutType >
        where <Self as Convertable>::OutType: HasAfEnum;
}
...
macro_rules! overloaded_binary_func {
    ($doc_str: expr, $fn_name: ident, $help_name: ident, $ffi_name: ident) => (
        ...
        pub fn $fn_name<T, U> (arg1: &T, arg2: &U, batch: bool)
            -> Array< <<T as Convertable>::OutType as ImplicitPromote<<U as Convertable>::OutType>>::Output >
                where T: Convertable,
                      U: Convertable,
...
fn clamp_helper<X, Y>(inp: &Array<X>, lo: &Array<Y>, hi: &Array<Y>, batch: bool)
    -> Array< <X as ImplicitPromote<Y>>::Output >
        where X: HasAfEnum + ImplicitPromote<Y>,
...
///
/// The trait `Convertable` essentially translates to a scalar native type on rust or Array.
pub fn clamp<T, C>(input: &Array<T>, arg1: &C, arg2: &C, batch: bool)
    -> Array< <T as ImplicitPromote<<C as Convertable>::OutType>>::Output >
        where T: HasAfEnum + ImplicitPromote<<C as Convertable>::OutType>,
              C: Convertable,
...
macro_rules! arith_scalar_func {
    ($rust_type: ty, $op_name:ident, $fn_name: ident) => (
        impl<'f, T> $op_name<$rust_type> for &'f Array<T>
            where T: HasAfEnum + ImplicitPromote<$rust_type>,
...
impl<T> Neg for Array<T>
    where T: HasAfEnum+Zero+ConstGenerator,
